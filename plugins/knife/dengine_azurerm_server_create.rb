...
class Chef
  class Knife
    class DengineAzurermServerCreate < Knife
      ...
      banner "knife dengine azurerm server create (options)"
      ...
      def run
        ...
        begin
          ...
          ip = service.show_server(config[:azure_vm_name], locate_config_value(:azure_resource_group_name))
          puts "The public IP of the VM is: #{ip}"   
          node_name = get_node(config[:azure_vm_name])
          node_name.each do |node|
          node.default_unless['cloud']['public_ipv4'] = "#{ip}"
          node.default_unless['cloud']['provider'] = 'azure'
          node.save
          end
        rescue => error
