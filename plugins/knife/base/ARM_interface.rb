...
require "#{File.dirname(__FILE__)}/dengine_azure_arm_vnet_config"
require "#{File.dirname(__FILE__)}/dengine_azure_arm_deployment_template"
require "#{File.dirname(__FILE__)}/dengine_azure_arm_base"
...
module Azure
  class ResourceManagement
    class ARMInterface < DengineAzureInterface
      include Azure::ARM::DengineAzureArmBase
      include Azure::ARM::DengineAzureArmDeploymentTemplate
      include Azure::ARM::DengineAzureArmVnetConfig
      ...
      def show_server(name, resource_group)
        ...
        return public_ip_data.ip_address
      end
      ...
      def availability_set_exist?(resource_group_name, availability_set_name)
        begin
          compute_management_client.availability_sets.get(resource_group_name, availability_set_name)
          return true
        rescue MsRestAzure::AzureOperationError => error
          if error.body
            err_json = JSON.parse(error.response.body)
